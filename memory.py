...
def load_memory(filepath: str = DEFAULT_MEMORY_FILE) -> dict:
    ...
    try:
        ...
        print(
            f"Warning: {filepath} was corrupted. Starting with empty memory."
        )
        return {}


def save_memory(
    memory_data: dict, filepath: str = DEFAULT_MEMORY_FILE
) -> None:
...
def add_concept(
    ...
    if concept_name in memory_data:
        raise ValueError(f"Concept '{concept_name}' already exists.")
...
def delete_concept(concept_name: str, filepath: str = DEFAULT_MEMORY_FILE) -> bool:
    """Removes a concept from the memory database.
    
    Returns True if the concept was deleted, False if it did not exist.
    """
    memory_data = load_memory(filepath)
    if concept_name in memory_data:
        del memory_data[concept_name]
        save_memory(memory_data, filepath)
        return True
    return False
...
def update_concept(
    concept_name: str, new_expected_answer: str, filepath: str = DEFAULT_MEMORY_FILE
) -> bool:
    """Updates the expected answer of a concept in the memory database.
    
    Returns True if the concept was updated, False if it did not exist.
    """
    memory_data = load_memory(filepath)
    if concept_name in memory_data:
        memory_data[concept_name]["expected_answer"] = new_expected_answer
        save_memory(memory_data, filepath)
        return True
    return False
