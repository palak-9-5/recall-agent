...
def handle_add_concept(filepath: str = MEMORY_FILE):
    ...
    concept_name = input("Enter the concept name (e.g., 'Python decorators'): ").strip()
    if not concept_name:
    ...
    expected_answer = input("Enter expected answer / key points to grade against: ").strip()
...
def handle_list_concepts(filepath: str = MEMORY_FILE):
    ...
    print(f"{'Concept Name':<22} | {'EF':<5} | {'Interval':<8} | {'Reps':<5} | {'Next Review':<11}")
    print("-" * 60)
...
def handle_review_session(filepath: str = MEMORY_FILE, is_simulation: bool = False):
    ...
    if not due_concepts:
        print("🎉 Hurrah! No reviews due at this moment. You are all caught up!")
        return
    ...
    for i, (name, details) in enumerate(due_concepts.items(), 1):
        ...
        if is_simulation:
            print(f"[MOCK DISCORD WEBHOOK] Sending message:\n{discord_message}")
        else:
        ...
        if is_simulation:
            ...
            if overlap >= 3 or (len(expected_words) > 0 and (overlap / len(expected_words)) >= 0.5):
                score = 5
            ...
            try:
                score, feedback = grader.grade_answer(question, expected, user_response, gemini_key)
            except Exception as e:
                print(f"\n❌ Grading Error: {e}")
                print("Falling back to manual scoring.")
                while True:
                    try:
                        score = int(input("Please enter a manual score (0 = forgot, 5 = perfect): "))
                        if 0 <= score <= 5:
        ...
        db[name]["history"].append(review_entry)
        
        # Save memory intermediate to disk so progress isn't lost if aborted
        memory.save_memory(db, filepath)
        
        print(f"Concept scheduled. Next review in {new_interval} day(s) ({next_review_str}).\n")
        
    print("Session complete! Memory database successfully updated.")

def run_simulation_mode():
    print_header("Starting Simulation Mode")
    print("This mode simulates the spaced repetition process using a temporary database.")
    print("You do NOT need active Gemini API keys or Discord webhooks for this mode.\n")
